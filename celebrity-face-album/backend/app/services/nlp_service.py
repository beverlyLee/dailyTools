...
class NLPService:
    ...
    def _extract_birth_date(self, text: str) -> Optional[datetime]:
        """提取出生日期"""
        # 匹配YYYY-MM-DD格式
    ...
    def _extract_education(self, text: str) -> Dict[str, Any]:
        ...
        if match:
            try:
                info["graduation_date"] = datetime(year=int(match.group(1)), month=7, day=1)
            except ValueError:
                pass
        ...
        if not experiences:
            ...
            for para in paragraphs[:3]:  # 只取前3段
                ...
                experiences.append({
                    ...
                    "end_date": end_date,
                    "is_current": 1 if is_current else 0,
                    "description": para[:200]  # 截取前200字符作为描述
                })
    ...
    def _extract_educations(self, text: str) -> List[Dict[str, Any]]:
        ...
        # 如果没有找到结构化的教育经历，尝试从段落中提取
        if not educations:
            paragraphs = re.split(r'\n\s*\n', education_section)
            for para in paragraphs[:2]:  # 只取前2段
                para = para.strip()
                if not para:
                    continue
