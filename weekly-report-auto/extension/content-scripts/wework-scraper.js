(function() {
  ...
  class WeWorkScraper {
    ...
    extractConversationInfo() {
      const titleElement = document.querySelector('.header-info__name, .chat-header__name, [class*="title"]');
      const memberElements = document.querySelectorAll('.chat-member-item, [class*="member"]');
    ...
    checkIfSelf(element) {
      ...
      const alignRight = element.style.justifyContent === 'flex-end' || 
    ...
    detectMessageType(element) {
      ...
      const hasFile = element.querySelector('[class*="file"], [class*="attachment"]');
      if (hasFile) {
        return 'file';
      }

      const hasEmoji = element.querySelector('[class*="emoji"]');
      if (hasEmoji && !element.textContent.trim()) {
        return 'emoji';
      }
    ...
    generateMessageId(element, index) {
      ...
      return `wework_${timestamp}_${index}`;
    }
  ...
  const scraper = new WeWorkScraper();
  window.__scraper__ = scraper;

  console.log('企业微信数据采集脚本已加载');
})();
