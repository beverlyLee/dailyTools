...
pub struct Comparison {
    pub id: i64,
    pub left_file_name: String,
    pub right_file_name: String,
...
impl Database {
    pub async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        let app_dir = dirs::data_local_dir()
            .ok_or("无法获取应用数据目录")?
            .join("personal-productivity-text-tools");
    ...
    pub async fn create_comparison(
        ...
        let id = sqlx::query(
            r#"
            INSERT INTO comparisons (left_file_name, right_file_name, left_content, right_content, created_at)
            VALUES (?, ?, ?, ?, ?)
            "#
        )
        .bind(&left_file_name)
        .bind(&right_file_name)
        .bind(&left_content)
        .bind(&right_content)
        .bind(&now)
        .execute(&self.pool)
    ...
    pub async fn get_comparison(&self, id: i64) -> Result<Option<Comparison>, sqlx::Error> {
        sqlx::query_as!(
            Comparison,
            r#"
    ...
    pub async fn get_all_comparisons(&self) -> Result<Vec<Comparison>, sqlx::Error> {
        sqlx::query_as!(
            Comparison,
            r#"
            SELECT id, left_file_name, right_file_name, left_content, right_content, created_at
    ...
    pub async fn delete_comparison(&self, id: i64) -> Result<(), sqlx::Error> {
