import { Controller, Point, Bounds } from '../core';
...
export class PDFController extends Controller<PDFModel, PDFView> {
  ...
  async loadDocument(data: ArrayBuffer | string, fileName: string = 'document.pdf'): Promise<void> {
    try {
      ...
      this._model.setPDFDocument(pdfDoc, docInfo as any);
      await this._view.renderPage();

      (this.emit as any)('document:loaded', docInfo);
    } catch (error) {
  ...
  async nextPage(): Promise<boolean> {
    if (this._model.nextPage()) {
      await this._view.renderPage();
      (this.emit as any)('page:change', this._model.currentPage);
      return true;
    }
    return false;
  }
  ...
  async zoomIn(factor: number = 0.1): Promise<void> {
    ...
    await this._view.renderPage();
    (this.emit as any)('zoom:change', this._model.scale);
  }
  ...
  private finishDrawing(point: Point): void {
    ...
    const activeTool = this.activeToolId;
    if (activeTool === 'highlight' || activeTool === 'underline') {
      ...
      (this.emit as any)('annotation:added', annotation);
    }
  ...
  removeAnnotation(annotationId: string): boolean {
    ...
    if (success) {
      (this.emit as any)('annotation:removed', annotationId);
      this._view.renderAnnotations();
    }
    return success;
  }
