...
import { audiobookApi, UploadResponse } from '@/lib/api';

interface UploaderProps {
  onUploadSuccess: (audiobookId: number) => void;
}
...
export default function Uploader({ onUploadSuccess }: UploaderProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const result: UploadResponse = await audiobookApi.upload(selectedFile);
      
      setTimeout(() => {
        onUploadSuccess(result.audiobook_id);
      }, 500);
    } catch (err: any) {
    ...
    <div className="w-full max-w-2xl mx-auto">
      ...
      >
        ...
        <h3 className="text-lg font-semibold text-gray-700 mb-2">
          {isDragActive ? '释放文件以上传' : '拖放音频文件到这里'}
        </h3>
        <p className="text-gray-500 text-sm">
          或点击选择文件，支持 MP3、WAV 格式
        </p>
      </div>
          ...
          {isUploading && (
            <div className="mt-4">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className="bg-primary-600 h-2 rounded-full animate-pulse w-full"
                />
              </div>
            </div>
          )}
