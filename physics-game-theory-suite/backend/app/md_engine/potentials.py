...
class LennardJonesPotential:
    ...
    def compute_single_pair(self, r: float) -> Tuple[float, float]:
        if r >= self.cutoff:
            return 0.0, 0.0
        ...
        energy = 4 * self.epsilon * (self.sigma12 * inv_r12 - self.sigma6 * inv_r6) - self.shift_energy
        force = 24 * self.epsilon * inv_r * (
            2 * self.sigma12 * inv_r12 - self.sigma6 * inv_r6
        )
