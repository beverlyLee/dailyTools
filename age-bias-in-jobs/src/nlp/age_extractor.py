            ...
            for match in matches:
                ...
                if ptype == 'max':
                    ...
                    if '90后' in keyword:
                        ...
                        result['generation'] = 80
                        result['max_age'] = 45
                    elif '00后' in keyword:
                        result['generation'] = 00
                        result['max_age'] = 25
                ...
                elif ptype == 'bias_keyword':
                    result['has_bias_keyword'] = True

        return result

    def get_age_category(self, max_age: Optional[int]) -> str:
        if max_age is None:
            return '无年龄限制'
        elif max_age <= 25:
            return '25岁以下'
...
def calculate_age_bias_ratio(df, jd_column='job_description') -> Dict:
    extractor = AgeExtractor()
    results = extractor.batch_extract(df[jd_column].tolist())
    ...
    categories = {}
    for r in results:
        cat = extractor.get_age_category(r['max_age'])
