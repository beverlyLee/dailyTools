...
import { Ban, Search, Bell, BellOff, RefreshCw, Building2, AlertTriangle, Phone, MapPin, FileCheck } from 'lucide-vue-next';
import type { BlacklistedCompany } from '../../shared/types';

const companies = ref<BlacklistedCompany[]>([]);
...
async function loadBlacklist() {
  ...
  try {
    if (searchKeyword.value.trim()) {
      companies.value = await api.searchBlacklist(searchKeyword.value.trim());
    } else {
      companies.value = await api.getBlacklist();
    }
  } catch (e) {
...
function isSubscribed(companyId: string): boolean {
  return subscribedIds.value.has(companyId);
}
  ...
  <div class="space-y-4">
    ...
    <div v-else class="space-y-4">
      <div
        ...
        :key="company.id"
        class="bg-white rounded-2xl p-5 shadow-lg border-l-4 transition-all duration-200 hover:shadow-xl"
        :class="[
        ...
        <div class="flex items-start justify-between">
          <div class="flex-1">
            ...
            <div class="space-y-2 text-sm">
              ...
              <div class="flex items-start gap-2 text-red-600 bg-red-50 rounded-lg p-3 mt-3">
                <AlertTriangle class="w-4 h-4 text-red-500 flex-shrink-0 mt-0.5" />
                <div>
                  <p class="font-medium">列入原因</p>
                  <p class="text-red-700">{{ company.reason }}</p>
                </div>
              </div>
              <div class="flex items-center justify-between pt-2 border-t border-gray-100">
                <span class="text-gray-500 text-xs">列入日期：{{ formatDate(company.dateAdded) }}</span>
                <span
                  class="text-xs px-2 py-1 rounded-full"
                  :class="company.status === 'active' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'"
                >
                  {{ company.status === 'active' ? '黑名单中' : '已移除' }}
                </span>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            class="ml-4 p-2 rounded-lg transition-colors"
            :class="[
              selectedCompanies.includes(company.id)
          ...
          >
            <Bell v-if="selectedCompanies.includes(company.id)" class="w-5 h-5" />
