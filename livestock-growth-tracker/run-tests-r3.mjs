...
async function runTests() {
  console.log('=== 牲畜生长数字化管理系统 第三轮验收测试 ===\n')
  ...
  try {
    ...
    // ===== 步骤5: 免疫中心 - 重点验证疫苗名称下拉框 =====
    console.log('\n--- 步骤5: 免疫中心 - 验证疫苗名称下拉框修复 ---')
    ...
    if (vaccineForm) {
      ...
      console.log(`  选中的牲畜ID: ${selectedLivestockId}`)
      ...
      if (vaccineOptions.length > 1) {
        console.log('  ✓ 【修复验证通过】疫苗名称下拉框已正确显示选项！')
        console.log(`  疫苗数量: ${vaccineOptions.length - 1}个`)
        ...
        console.log(`  ✓ 接种记录保存成功，记录行数: ${recordRows.length}`)
      } else {
        logIssue('免疫中心-疫苗名称', `选择牲畜后疫苗名称下拉框仍无选项，只有${vaccineOptions.length}个空选项`, 'high')
      }
    }
      ...
      if (resultsPanel) {
        ...
        if (resultRows.length > 0) {
          ...
          if (passCount === resultRows.length && passRateText?.includes('100')) {
            console.log('  ✓ 【修复验证通过】通过率显示正确：100%，与全部通过一致！')
          } else if (passCount > 0 && passRateText?.includes('0%')) {
            logIssue('系统验证-通过率', `通过率仍显示0%，实际${passCount}/${resultRows.length}条通过`, 'high')
          } else if (passCount > 0) {
            const expectedRate = (passCount / resultRows.length * 100).toFixed(1)
            if (passRateText?.includes(expectedRate)) {
              console.log(`  ✓ 通过率显示正确: ${expectedRate}%`)
            } else {
    ...
    const gSelect = await page.$('.livestock-select')
    const gOpts = await gSelect.$$('option')
    if (gOpts.length > 1) {
      await gSelect.selectOption({ index: 1 })
      await page.waitForTimeout(2000)
    ...
    await takeScreenshot(page, '09_growth_input')
    ...
    await takeScreenshot(page, '11_search_filter')
  ...
  fs.writeFileSync(path.join(IMG_DIR, 'test-report-r3.json'),
