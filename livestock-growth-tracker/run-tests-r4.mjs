...
async function runTests() {
  console.log('=== 牲畜生长数字化管理系统 第四轮验收测试 ===\n')
  ...
  try {
    ...
    if (opts.length > 1) {
      ...
      else logIssue('生长对标', `FCR值未计算: ${fcrValue}`, 'high')
      ...
      if (weightRows.length > 0) {
        ...
        else logIssue('生长对标', '偏离度渲染异常', 'high')
      }
    }
    ...
    if (vaccineForm) {
      // 选择第一个牲畜
      const livestockOptions = await page.$$eval('#v-livestock option', opts => opts.map(o => ({ value: o.value, text: o.textContent?.substring(0, 20) })))
      console.log(`  牲畜选项(前3): ${JSON.stringify(livestockOptions.slice(0, 3))}`)

      if (livestockOptions.length > 1) {
        const firstOptionValue = livestockOptions[1].value
        console.log(`  第一个牲畜value: "${firstOptionValue}" (${typeof firstOptionValue})`)

        await page.selectOption('#v-livestock', firstOptionValue)
        await page.waitForTimeout(2000)

        const selectedId = await page.inputValue('#v-livestock')
        console.log(`  选中后ID: "${selectedId}"`)
        ...
        if (vaccineOptions.length > 1) {
          console.log(`  ✓ 【修复验证通过】疫苗名称下拉框已正确显示${vaccineOptions.length - 1}个选项！`)
          ...
          await page.click('.sub-tab:nth-child(2)')
          await page.waitForTimeout(1000)
          const recordRows = await page.$$('.data-table tbody tr')
          console.log(`  ✓ 接种记录保存成功，记录行数: ${recordRows}`)
        } else {
          logIssue('免疫中心-疫苗名称', `选择牲畜后疫苗名称下拉框仍无选项，只有${vaccineOptions.length}个选项：${JSON.stringify(vaccineOptions)}`, 'high')
        }
      }
    }
    ...
    if (validateBtn) {
      ...
      if (resultsPanel) {
        ...
        const isFailClass = passRateClass?.includes('fail')
        console.log(`  通过率class: pass=${isPassClass}, fail=${isFailClass}`)
        ...
        if (resultRows.length > 0) {
          ...
          const expectedRate = passCount === resultRows.length ? '100' : (passCount / resultRows.length * 100).toFixed(1)
          const actualHasRate = passRateText?.includes(expectedRate) || passRateText?.includes(String(Math.floor(passCount / resultRows.length * 100)))

          if (passCount === resultRows.length && passRateText?.includes('100')) {
            console.log(`  ✓ 【修复验证通过】通过率正确显示100%，与全部通过一致！`)
          } else if (passCount === resultRows.length && !passRateText?.includes('100')) {
            logIssue('系统验证-通过率', `通过率仍显示错误，${passCount}/${resultRows.length}条全部通过，但显示: "${passRateText}"（未包含100%），class为${isPassClass ? 'pass' : 'fail'}`, 'high')
          } else if (passCount > 0 && actualHasRate) {
            console.log(`  ✓ 通过率显示正确: ${passRateText}`)
          } else {
            logIssue('系统验证-通过率', `通过率显示不一致，期望${expectedRate}%，实际${passRateText}`, 'medium')
          }
        }
      }
    }
    ...
    await takeScreenshot(page, '08_livestock_create')
    ...
    await takeScreenshot(page, '09_search_filter')
    ...
    await takeScreenshot(page, '10_growth_from_detail')
  ...
  fs.writeFileSync(path.join(IMG_DIR, 'test-report-r4.json'),
