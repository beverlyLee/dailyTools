...
from PyQt5.QtWidgets import (
    ...
    QLabel, QComboBox, QPushButton, QLineEdit, QTextEdit, QTableWidget,
    QTableWidgetItem, QHeaderView, QSplitter, QGroupBox, QMessageBox,
...
class PacketSnifferUI(QMainWindow):
    def __init__(self):
        ...
        self.packet_id_map: Dict[int, int] = {}
    ...
    def init_ui(self):
        ...
        self.status_bar.showMessage("就绪 - 选择网卡后点击'开始抓包'")
    ...
    def start_sniffing(self):
        ...
        try:
            ...
            self.status_bar.showMessage(f"抓包中 - {self.interface_combo.currentText()}")
    ...
    def stop_sniffing(self):
        try:
            ...
            captured_count = len(self.sniffer.get_captured_packets())
            self.log_message(f"抓包已停止，共捕获 {captured_count} 个数据包")
            self.status_bar.showMessage(f"已停止 - 共捕获 {captured_count} 个数据包")
    ...
    def update_packet_table(self):
        if self.sniffer.is_sniffing:
            self.status_bar.showMessage(
                f"抓包中 - 已捕获 {len(self.packet_list)} 个数据包"
            )
    ...
    def save_to_database(self):
        ...
        try:
            ...
            for packet in self.packet_list:
                self.database.insert_packet(packet)
                count += 1
