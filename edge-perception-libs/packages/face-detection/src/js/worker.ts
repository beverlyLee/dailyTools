...
async function handleInit(message: FaceDetectorWorkerMessage): Promise<void> {
  try {
    const importScriptsFunc = (globalThis as any).importScripts;
    
    if (importScriptsFunc) {
      try {
        importScriptsFunc('./pkg/edge_face_detection.js');
...
function handleSetOptions(message: FaceDetectorWorkerMessage): void {
  if (message.options) {
    applyOptions(message.options);
  }
}

function handleDetect(message: FaceDetectorWorkerMessage): void {
  ...
  try {
    ...
    const results: FaceDetectionResult[] = wasmResults.map((wasmResult) => ({
      bbox: {
        x: wasmResult.bbox.x,
        y: wasmResult.bbox.y,
...
function onMessageHandler(event: MessageEvent<FaceDetectorWorkerMessage>): void {
