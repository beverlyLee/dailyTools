...
def run():
    with sync_playwright() as p:
        ...
        print("[2] 初始状态截图(第5轮)...", flush=True)
        try:
            page.screenshot(path=f"{IMG_DIR}/r5_initial.png", timeout=15000)
            results["screenshots"].append("r5_initial.png")
        ...
        time.sleep(8)
        try:
            page.screenshot(path=f"{IMG_DIR}/r5_grown.png", timeout=15000)
            results["screenshots"].append("r5_grown.png")
            print("    截图成功", flush=True)
        except Exception as e:
            print(f"    截图失败: {e}", flush=True)
        ...
        time.sleep(8)
        try:
            page.screenshot(path=f"{IMG_DIR}/r5_fully_grown.png", timeout=15000)
            results["screenshots"].append("r5_fully_grown.png")
            print("    截图成功", flush=True)
        except Exception as e:
            print(f"    截图失败: {e}", flush=True)
        ...
        try:
            ...
            time.sleep(2)
            page.screenshot(path=f"{IMG_DIR}/r5_after_reset.png", timeout=15000)
            results["screenshots"].append("r5_after_reset.png")
        ...
        time.sleep(12)
        try:
            page.screenshot(path=f"{IMG_DIR}/r5_regrown.png", timeout=15000)
            results["screenshots"].append("r5_regrown.png")
            print("    截图成功", flush=True)
        except Exception as e:
            print(f"    截图失败: {e}", flush=True)
        ...
        try:
            ...
            time.sleep(2)
            page.screenshot(path=f"{IMG_DIR}/r5_after_R.png", timeout=15000)
            results["screenshots"].append("r5_after_R.png")
            print("    切换测试成功", flush=True)
        except Exception as e:
            print(f"    切换测试失败: {e}", flush=True)

        print("[8] 再次重置并生长(第2次)...", flush=True)
        try:
            page.keyboard.press("Space")
            time.sleep(2)
            time.sleep(12)
            page.screenshot(path=f"{IMG_DIR}/r5_regrown2.png", timeout=15000)
            results["screenshots"].append("r5_regrown2.png")
            print("    截图成功", flush=True)
        except Exception as e:
            print(f"    截图失败: {e}", flush=True)
    ...
    with open(f"{IMG_DIR}/test_results_r5.json", "w", encoding="utf-8") as f:
