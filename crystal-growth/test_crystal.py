...
def run():
    with sync_playwright() as p:
        ...
        try:
            page.screenshot(path=f"{IMG_DIR}/crystal_initial.png", timeout=15000)
            results["screenshots"].append("crystal_initial.png")
        ...
        time.sleep(8)
        try:
            page.screenshot(path=f"{IMG_DIR}/crystal_grown.png", timeout=15000)
            results["screenshots"].append("crystal_grown.png")
            print("    截图成功", flush=True)
        except Exception as e:
            print(f"    截图失败: {e}", flush=True)

        print("[4] 测试空格键重置...", flush=True)
        try:
            page.keyboard.press("Space")
            time.sleep(2)
            page.screenshot(path=f"{IMG_DIR}/crystal_after_reset.png", timeout=15000)
            results["screenshots"].append("crystal_after_reset.png")
            print("    重置测试成功", flush=True)
        except Exception as e:
            print(f"    重置测试失败: {e}", flush=True)

        print("[5] 重置后重新生长...", flush=True)
        time.sleep(8)
        try:
            page.screenshot(path=f"{IMG_DIR}/crystal_regrown.png", timeout=15000)
            results["screenshots"].append("crystal_regrown.png")
            print("    截图成功", flush=True)
        except Exception as e:
            print(f"    截图失败: {e}", flush=True)

        print("[6] 测试 R 键切换...", flush=True)
        try:
            page.keyboard.press("KeyR")
            time.sleep(2)
            page.screenshot(path=f"{IMG_DIR}/crystal_toggle.png", timeout=15000)
            results["screenshots"].append("crystal_toggle.png")
            print("    切换测试成功", flush=True)
        except Exception as e:
            print(f"    切换测试失败: {e}", flush=True)

        print("[7] 检测 WebGL 和 canvas...", flush=True)
        try:
    ...
    with open(f"{IMG_DIR}/test_results.json", "w", encoding="utf-8") as f:
