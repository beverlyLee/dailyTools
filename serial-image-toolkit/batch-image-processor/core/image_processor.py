...
class ImageProcessor:
    ...
    def convert_format(self, img: np.ndarray, 
                        output_format: str, 
                        output_path: str,
                        quality: int = 90) -> bool:
        if not output_path.lower().endswith(output_format.lower()):
            base = Path(output_path).parent / Path(output_path).stem
            output_path = f"{base}.{output_format.lower()}"
        
        return self.save_image(img, str(output_path), quality)
    ...
    def get_supported_images(self, directory: str) -> List[str]:
        images = []
