...
async def test_matchmaking_app():
    ...
    async with async_playwright() as p:
        ...
        try:
            await page.goto("http://localhost:8504", timeout=60000)
            await page.wait_for_load_state("networkidle", timeout=30000)
            await asyncio.sleep(15)
            
            await page.screenshot(path=os.path.join(IMG_DIR, "r3_1_homepage.png"), full_page=True)
            screenshots.append("r3_1_homepage.png - 首页加载")
            ...
            if await selectbox_click_area.count() > 0:
                ...
                try:
                    ...
                    if await beijing_option.count() > 0:
                        ...
                        await page.screenshot(path=os.path.join(IMG_DIR, "r3_2_beijing_wordcloud.png"), full_page=True)
                        screenshots.append("r3_2_beijing_wordcloud.png - 北京词云图")
                ...
                try:
                    ...
                    if await shanghai_option.count() > 0:
                        ...
                        await asyncio.sleep(8)
                        await page.screenshot(path=os.path.join(IMG_DIR, "r3_3_shanghai_wordcloud.png"), full_page=True)
                        screenshots.append("r3_3_shanghai_wordcloud.png - 上海词云图")
            ...
            if await stats_tab.count() > 0:
                ...
                await page.screenshot(path=os.path.join(IMG_DIR, "r3_4_statistics.png"), full_page=True)
                screenshots.append("r3_4_statistics.png - 统计数据标签页")
            else:
            ...
            if await raw_data_tab.count() > 0:
                ...
                await page.screenshot(path=os.path.join(IMG_DIR, "r3_5_raw_data.png"), full_page=True)
                screenshots.append("r3_5_raw_data.png - 原始数据标签页")
                ...
                if missing_headers:
                    errors.append(f"原始数据表缺少友好表头: {', '.join(missing_headers[:3])}")
                
                if "selected_city" in page_content:
                    errors.append("原始数据表仍包含数据库字段名 'selected_city'")
            else:
            ...
            if await wordcloud_tab.count() > 0:
                ...
                await page.screenshot(path=os.path.join(IMG_DIR, "r3_6_wordcloud_tab.png"), full_page=True)
                screenshots.append("r3_6_wordcloud_tab.png - 词云图标签页")
            else:
            ...
            try:
                await page.screenshot(path=os.path.join(IMG_DIR, "r3_error_load.png"), full_page=True)
                screenshots.append("r3_error_load.png - 加载错误截图")
            except:
                pass
