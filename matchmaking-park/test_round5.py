...
async def test_matchmaking_app():
    ...
    async with async_playwright() as p:
        ...
        try:
            await page.goto("http://localhost:8506", timeout=60000)
            await page.wait_for_load_state("networkidle", timeout=30000)
            await asyncio.sleep(15)
            
            await page.screenshot(path=os.path.join(IMG_DIR, "r5_1_homepage.png"), full_page=True)
            screenshots.append("r5_1_homepage.png - 首页加载")
            
            page_title = await page.title()
            if "城市公园相亲角" not in page_title:
                errors.append(f"页面标题不正确: {page_title}")
            
            data_source_radio = page.locator('[data-testid="stSidebar"] [data-testid="stRadio"]')
            if await data_source_radio.count() > 0:
                screenshots.append("数据源选择组件已找到")
            else:
                errors.append("未找到数据源选择组件")
            ...
            if await selectbox_click_area.count() > 0:
                ...
                try:
                    ...
                    if await beijing_option.count() > 0:
                        ...
                        await page.screenshot(path=os.path.join(IMG_DIR, "r5_2_beijing_wordcloud.png"), full_page=True)
                        screenshots.append("r5_2_beijing_wordcloud.png - 北京词云图")
                ...
                try:
                    ...
                    if await shanghai_option.count() > 0:
                        ...
                        await asyncio.sleep(8)
                        await page.screenshot(path=os.path.join(IMG_DIR, "r5_3_shanghai_wordcloud.png"), full_page=True)
                        screenshots.append("r5_3_shanghai_wordcloud.png - 上海词云图")
            ...
            if await stats_tab.count() > 0:
                ...
                await page.screenshot(path=os.path.join(IMG_DIR, "r5_4_statistics.png"), full_page=True)
                screenshots.append("r5_4_statistics.png - 统计数据标签页")
            else:
            ...
            if await raw_data_tab.count() > 0:
                ...
                await page.screenshot(path=os.path.join(IMG_DIR, "r5_5_raw_data.png"), full_page=True)
                screenshots.append("r5_5_raw_data.png - 原始数据标签页")
                ...
                tech_headers = ["has_hukou", "has_education", "education_keywords", "ocr_confidence"]
                found_tech_headers = [h for h in tech_headers if h in page_content]
                if found_tech_headers:
                    errors.append(f"原始数据表仍包含数据库字段名: {', '.join(found_tech_headers)}")
                
                if "是" not in page_content and "否" not in page_content:
                    errors.append("布尔值未转换为'是/否'显示")
                
                if "%" not in page_content:
                    errors.append("OCR置信度未显示为百分比格式")
            else:
                errors.append("未找到'原始数据'标签页")
            
            pipeline_tab = page.get_by_text("数据管道")
            if await pipeline_tab.count() > 0:
                await pipeline_tab.first.click()
                await asyncio.sleep(5)
                await page.screenshot(path=os.path.join(IMG_DIR, "r5_6_pipeline.png"), full_page=True)
                screenshots.append("r5_6_pipeline.png - 数据管道标签页")
                
                page_content = await page.content()
                if "Scrapy" not in page_content:
                    errors.append("数据管道页面未显示Scrapy相关内容")
            else:
                errors.append("未找到'数据管道'标签页")
            
            wordcloud_tab = page.get_by_text("词云图")
            if await wordcloud_tab.count() > 0:
                await wordcloud_tab.first.click()
                await asyncio.sleep(4)
                await page.screenshot(path=os.path.join(IMG_DIR, "r5_7_wordcloud_tab.png"), full_page=True)
                screenshots.append("r5_7_wordcloud_tab.png - 词云图标签页")
            else:
            ...
            try:
                await page.screenshot(path=os.path.join(IMG_DIR, "r5_error_load.png"), full_page=True)
                screenshots.append("r5_error_load.png - 加载错误截图")
            except:
                pass
