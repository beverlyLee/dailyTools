...
def run_tests():
    print("\n" + "="*80)
    print("🌊 深海灯笼 (deep-sea-lantern) - 第2轮测试")
    print("="*80 + "\n")
    ...
    with sync_playwright() as p:
        ...
        try:
            time.sleep(1)
            page.screenshot(path=f"{img_dir}/r2_01_before_mouse_move.png")
            log_test("2. 初始状态截图", True, "已保存（需人工验证是否全黑）")
            test_results.append(("初始状态截图", True, "需人工验证"))
        except Exception as e:
            log_test("2. 初始状态截图", False, str(e))
            test_results.append(("初始状态截图", False, str(e)))
            issues.append(f"初始状态截图失败: {str(e)}")
        ...
        try:
            ...
            if box:
                ...
                time.sleep(1)
                page.screenshot(path=f"{img_dir}/r2_02_after_mouse_move.png")
        ...
        try:
            ...
            if box:
                ...
                for i, (ox, oy) in enumerate([(0, 0), (-200, -100), (200, 100)]):
                    page.mouse.move(center_x + ox, center_y + oy)
                    time.sleep(0.5)
                    page.screenshot(path=f"{img_dir}/r2_03_light_visual_{i}.png")
                
                log_test("4. 灯光视觉反馈", True, "不同位置已截图")
                test_results.append(("灯光视觉反馈", True, "已截图"))
            else:
                log_test("4. 灯光视觉反馈", False, "无法进行测试")
                test_results.append(("灯光视觉反馈", False, "无法测试"))
        except Exception as e:
            log_test("4. 灯光视觉反馈", False, str(e))
            test_results.append(("灯光视觉反馈", False, str(e)))
        
        # ========== 测试5: 雾效和沉浸感 ==========
        try:
            page.screenshot(path=f"{img_dir}/r2_04_fog_and_atmosphere.png")
            log_test("5. 雾效和沉浸感", True, "已截图（需人工验证）")
            test_results.append(("雾效和沉浸感", True, "需人工验证"))
        except Exception as e:
            log_test("5. 雾效和沉浸感", False, str(e))
            test_results.append(("雾效和沉浸感", False, str(e)))
