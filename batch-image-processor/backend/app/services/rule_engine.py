...
from app.models.database import RenameRule
from app.utils.exif_utils import EXIFUtils
...
class RuleEngine:
    ...
    def validate_rule(self) -> dict:
        ...
        if self.rule.use_date:
            valid_formats = ["%Y%m%d", "%Y-%m-%d", "%d%m%Y", "%d-%m-%Y", 
                           "%Y%m%d_%H%M%S", "%Y%m%d-%H%M%S"]
            if self.rule.date_format and self.rule.date_format not in valid_formats:
                try:
