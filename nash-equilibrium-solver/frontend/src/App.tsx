...
import type { ColDef, ValueSetterParams, CellStyleParams } from 'ag-grid-community';
...
import { nashEquilibriumApi, gameExamplesApi } from './services/api';
...
function App() {
  ...
  const loadGameExamples = async () => {
    try {
      const examples = await gameExamplesApi.getAll();
  ...
  const handleSolve = async () => {
    ...
    try {
      ...
      const response = await nashEquilibriumApi.solve(request);
      setResult(response);
    } catch (err) {
  ...
  return (
    <div className="container">
      <header className="header">
        <h1>🎮 纳什均衡求解器</h1>
        <p>输入双人博弈的收益矩阵，自动求解纯策略和混合策略纳什均衡</p>
      </header>
